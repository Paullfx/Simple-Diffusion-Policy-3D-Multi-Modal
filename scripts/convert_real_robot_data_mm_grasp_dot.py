...
def preprocess_point_cloud(points, extrinsics_matrix, use_cuda=True):
    num_points = 2048
...
def add_gaussian_noise_to_pointcloud(pointcloud, robot_state, std_dev=0.01):
    extrinsics_matrix = np.array([[0.05459751,  0.34765827, -0.93603036,  1.13547264],
                                    [ 0.99848749,  -0.01293582, 0.05343597,  -0.06979911],
                                    [0.00646914, -0.93753207, -0.3478387,   0.32493475],
                                    [ 0.,          0. ,         0.,          1.        ]])
    ...
    # Add Gaussian noise to the identified points
...
extrinsic_matrix_traj = np.array([[0.05459751,  0.34765827, -0.93603036,  1.13547264],
                                    [ 0.99848749,  -0.01293582, 0.05343597,  -0.06979911],
                                    [0.00646914, -0.93753207, -0.3478387,   0.32493475],
                                    [ 0.,          0. ,         0.,          1.        ]])

extrinsic_matrix_traj_v2 = np.array([[0.05459751,  0.34765827, -0.93603036,  1.13547264],
                                      [ 0.99848749,  -0.01293582, 0.05343597,  -0.04979911],
                                      [0.00646914, -0.93753207, -0.3478387,   0.31393475],
                                      [ 0.,          0. ,         0.,          1.        ]])

extrinsic_matrix_traj_v3 = np.array([[ 0.03909669,  0.37761151, -0.92513836,  1.14777914],
...
save_data_path = '/home/yxt/thesis/yirui/imitation/3D-Diffusion-Policy/3D-Diffusion-Policy/data/1-12-complete/pick-place-kinect-rs-dot.zarr'
...
for demo_dir in demo_dirs:
    ...
    for step_idx in range(demo_length):
        total_count += 1
        obs_image = demo['certainty_map'][step_idx]
        obs_image = np.where(obs_image > 0.7, 1, 0)
        if np.sum(obs_image == 1) > 700:
            coords = np.argwhere(obs_image[:, :] == 1)
            avg_coords = np.mean(coords, axis=0).astype(int)
            center_x, center_y = avg_coords[0], avg_coords[1]
            
            # Create a new image of the same size as obs_image
            new_image = np.zeros_like(obs_image)
            
            # Define the circular region around the center with radius 20
            for x in range(obs_image.shape[0]):
                for y in range(obs_image.shape[1]):
                    if np.sqrt((x - center_x)**2 + (y - center_y)**2) <= 30:
                        new_image[x, y] = 1
        else:
            new_image = np.zeros_like(obs_image)
        ...
        new_image = preproces_image(new_image)
        ...
        if mask_pc:
            ...
            if 'traj' in demo_dir and 'v3'  in demo_dir:
                obs_pointcloud= preprocess_point_cloud(obs_pointcloud, extrinsic_matrix_traj_v3)
            elif 'traj' in demo_dir and 'v2' in demo_dir:
                obs_pointcloud = preprocess_point_cloud(obs_pointcloud, extrinsic_matrix_traj_v2)
            elif 'traj' in demo_dir and 'v2' not in demo_dir and 'v3' not in demo_dir:
                obs_pointcloud = preprocess_point_cloud(obs_pointcloud, extrinsic_matrix_traj)
            else:
                raise NotImplementedError('Unknown trajectory')
        action_arrays.append(action)
        point_cloud_arrays.append(obs_pointcloud)
        state_arrays.append(robot_state)
        img_arrays.append(new_image)
...
#for certainty map only 1 channel
if img_arrays.shape[1] == 1: 
...
zarr_data.create_dataset('certainty_map', data=img_arrays, chunks=img_chunk_size, dtype='uint8', overwrite=True, compressor=compressor)
