...
class ConditionalUnet1DShorter(nn.Module):
    def __init__(self, 
        input_dim,
        local_cond_dim=None,
        global_cond_dim=None,
        ...
        down_modules = nn.ModuleList([])
        dim_in_0, dim_out_0 = in_out[0]
        down_modules.append(Upsample1d(dim_in_0))
        for ind, (dim_in, dim_out) in enumerate(in_out):
            is_last = ind >= (len(in_out) - 1)
            down_modules.append(nn.ModuleList([
                ConditionalResidualBlock1D(
        ...
        self.last_down = Downsample1d(dim_out_0)
    ...
    def forward(self, 
        ...
        x = sample
        h = []
        x = self.down_modules[0](x)       
        for idx, (resnet, downsample) in enumerate(self.down_modules[1:]):
            if self.use_down_condition:
                x = resnet(x, global_feature)
                if idx == 0 and len(h_local) > 0:
                    x = x + h_local[0]
