...
class SimpleDP3MMProjFlag(BasePolicy):
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        adopt_flag = torch.zeros(size=(bs, 1), device=self.device, dtype=torch.bool)
        ...
        if self.adapt_height:
            ...
            # obs_dict['agent_pos'][:, :, 2] = obs_dict['agent_pos'][:, :, 2] - (self.end_effector_length - self.train_gripper_length)
            # only change the height when adopt_flag is True
            obs_dict['agent_pos'][adopt_flag, :, 2] = obs_dict['agent_pos'][adopt_flag, :, 2] - (self.end_effector_length - self.train_gripper_length)
        ...
        nsample = self.conditional_sample(
            ...
            obs_dict=obs_dict, Da=Da, start=start, end=end, adopt_flag=adopt_flag,
            **self.kwargs)
    ...
    def compute_loss(self, batch):
        # normalize input, adapt height is false
        if self.adapt_height:
    ...
    def projection(self, pose, x_init, device, adopt_flag):
        ...
        for b in range(B):
            ...
            lbg = ca.vertcat(0.02, lbg2)   # Lower bound for g1 and g2
