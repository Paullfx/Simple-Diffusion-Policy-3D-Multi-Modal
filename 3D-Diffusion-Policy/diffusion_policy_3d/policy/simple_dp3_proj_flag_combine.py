...
class SimpleDP3ProjFlagCombine(BasePolicy):
    def __init__(self, 
            shape_meta: dict,
    ...
    def conditional_sample(self, 
            ...
            generator=None, cur_pose=None, Da=None, start=None, end=None, adopt_flag=None, adopt_flag_tar=None, tar_act=None,
        ...
        for t in scheduler.timesteps:
            ...
            if infernce_step >= 7:
                ...
                if tar_act is not None:
                    action = self.projection_tar(cur_pose, action, condition_data.device, adopt_flag_tar, tar_act)
                action = self.projection(cur_pose, action, condition_data.device, adopt_flag)
    ...
    def predict_action(self, target_act: np.array, robot_cur_pose: np.array, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        """
        obs_dict: must include "obs" key
        result: must include "action" key
        """
        if robot_cur_pose.shape[0] != obs_dict['agent_pos'].shape[0]:
            cur_pose = obs_dict['agent_pos']
            print(f"robot_cur_pose shape: {robot_cur_pose.shape}, obs_dict['agent_pos'] shape: {obs_dict['agent_pos'].shape}")
        else:
            cur_pose = robot_cur_pose
        bs = cur_pose.shape[0]
        self.adopt_flag = torch.zeros(size=(bs, 1), device=self.device, dtype=torch.bool)
        self.adopt_flag_tar = torch.zeros(size=(bs, 1), device=self.device, dtype=torch.bool)

        # Define the rectangle bounds
        x_min, y_min = 0.48, -0.118
        x_max, y_max = 0.58, 0.014
        z_max = 0.15

        for i in range(bs):
            x, y, z = cur_pose[i, -1, 0], cur_pose[i, -1, 1], cur_pose[i, -1, 2] # x and y at the last timestep
            if x_min <= x <= x_max and y_min <= y <= y_max and z - self.end_effector_length < z_max:
                self.adopt_flag[i] = True
                self.adopt_flag_tar[i] = True
    
        if target_act is None:
            tar_act = None
        else:
            tar_act = torch.tensor(target_act, device=self.device, dtype=torch.float32)

        self.adopt_flag_tar = self.adopt_flag_tar.squeeze(-1)
        ...
        nsample = self.conditional_sample(
            ...
            cur_pose=cur_pose, Da=Da, start=start, end=end, adopt_flag=self.adopt_flag, adopt_flag_tar=self.adopt_flag_tar, tar_act=tar_act,
            **self.kwargs)
    ...
    def projection(self, pose, x_init, device, adopt_flag):
        ...
        for b in range(B):
            ...
            cumulative_pose = np.squeeze(cumulative_pose, axis=0)
    ...
    def projection_tar(self, pose, x_init, device, adopt_flag_tar, tar_act):
        if isinstance(x_init, torch.Tensor):
            x_init_np = x_init.detach().cpu().numpy()
            torch_dtype = x_init.dtype
        else:
        ...
        if isinstance(tar_act, torch.Tensor):
            tar_act_np = tar_act.detach().cpu().numpy()
        else:
            tar_act_np = tar_act
        ...
        for b in range(B):
            ...
            gripper_close = False
            if (x_init_np[b, 0, 7] - x_init_np[b, 0, 6]) >= 0.5:
                gripper_close = True

            if gripper_close and adopt_flag_tar[b]:
                self.first_close = True
            
            if not self.first_close:
                x_proj[b, :, :] = x_init_np[b, :, :]
                continue
            ...
            z_init_vars = x_init_np[b, :, 2].copy()
            
            target_x = tar_act_np[b, 0, 0]
            target_y = tar_act_np[b, 0, 1]
            target_z = tar_act_np[b, 0, 2]

            x_vars = ca.SX.sym('x', T)
            y_vars = ca.SX.sym('y', T)
            z_vars = ca.SX.sym('z', T)
            
            obj = ca.sumsqr(x_vars - x_init_vars) + ca.sumsqr(y_vars - y_init_vars) + ca.sumsqr(z_vars - z_init_vars)

            final_x = cumulative_pose[0] + ca.sum1(x_vars)
            final_y = cumulative_pose[1] + ca.sum1(y_vars)
            final_z = cumulative_pose[2] + ca.sum1(z_vars)
            
            g = ca.sqrt((final_x - target_x)**2 + (final_y - target_y)**2 + (final_z - target_z)**2)
            lbg = [0] 
            ubg = [0.005] 
            
            nlp = {'x': ca.vertcat(x_vars, y_vars, z_vars), 'f': obj, 'g': g}
            
            solver = ca.nlpsol("solver", "ipopt", nlp,
                   {"print_time": False, "ipopt": {"print_level": 0}})
            
            x0 = ca.vertcat(x_init_vars, y_init_vars, z_init_vars)
            sol = solver(x0=x0, lbg=lbg, ubg=ubg)
            sol_vec = sol["x"].full().ravel()         

            x_proj_vars = sol_vec[0:T]
            y_proj_vars = sol_vec[T : 2*T]
            z_proj_vars = sol_vec[2*T : 3*T]           # ← NEW

            x_proj[b, :, :]  = x_init_np[b, :, :]      # keep the rest unchanged
            x_proj[b, :, 0]  = x_proj_vars
            x_proj[b, :, 1]  = y_proj_vars
            x_proj[b, :, 2]  = z_proj_vars             # ← store the optimised z
            x_proj[b, :, 6:] = tar_act_np[b, 0, 6:]    
